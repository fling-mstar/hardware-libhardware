...
typedef struct hwc_layer_1 {
    ...
    union {
        ...
        struct {
            ...
            // MStar Android Patch Begin
            /* save previous frame to prevent to send same display info */
            hwc_rect_t prevFrame;
            char* name;
            hwc_rect_t activeBufferRegion;
            hwc_rect_t osdRegion;
            /* the required usage for this layer */
            uint32_t VendorUsageBits;
...
typedef struct hwc_composer_device_1 {
    ...
    // MStar Android Patch Begin
    int64_t (*getRefreshPeriod)(struct hwc_composer_device_1* dev);
#ifdef BUILD_FOR_STB
    void (*reSetPanelSize)(struct hwc_composer_device_1* dev, int width, int height);
#endif
    int (*require4k2kOutputTiming)(struct hwc_composer_device_1* dev,int enable);
#ifdef BUILD_MSTARTV_MI
    int (*setOSDRegionSize)(struct hwc_composer_device_1* dev, int width, int height);
    int (*switchOSDTiming)(struct hwc_composer_device_1 *dev,
                    size_t numDisplays, hwc_display_contents_1_t** displays);
#endif
    int (*getCurOpTiming)(struct hwc_composer_device_1* dev, int* width, int* height);
    int (*recalc3DTransform)(struct hwc_composer_device_1* dev, hwc_3d_param_t* params, hwc_rect_t frame);
    int (*getUrsaVsersion)(struct hwc_composer_device_1* dev);
    int (*isStbTarget)(struct hwc_composer_device_1* dev);
    int (*isOsdNeedResize)(struct hwc_composer_device_1* dev);
    // MStar Android Patch End
    /*
